...
BEGIN_MESSAGE_MAP(CZipArcDoc, CDocument)
	...
	ON_UPDATE_COMMAND_UI(ID_COMMAND_SETPASSWORD, OnUpdateCommandSetPassword )
...
void CZipArcDoc::OnUpdateCommandSetPassword( CCmdUI* pCmdUI )
{
	pCmdUI->Enable( false );
}
...
void EncryptDecryptFile( const char* pszFile, std::vector<int> vKeys )
{
	...
	if( pFile )
	{
		...
		if( buffer )
		{
			...
			if( pFile )
			{
				auto path = std::filesystem::path( pszFile );

				//Encrypt File before Add
				for( unsigned i = 0; i < size; i++ )
					buffer[i] ^= vKeys[i%vKeys.size()];
...
UINT CZipArcDoc::Add( LPVOID pParam )
{
	...
	for (i = 0; i < iSize; i++)
	{
		...
		TRY
		{
			...
			if( ai )
			{
				std::string filename( ai->m_szName );
				...
				if( path.has_filename() && path.has_extension() )
				{
					...
					strOldFileName = std::string( ai->m_szName );
					strNewFileName = std::string( ai->m_szName ) + ".bkp";
					std::filesystem::copy_file( std::string( ai->m_szName ), strNewFileName );
			...
			if (bAdd)
				bAddToReport = !zip.AddNewFile(zanfi, strComment.c_str());
			...
			else if( ai && strOldFileName.length() && strNewFileName.length() )
			{
				//Recover Old File
				std::filesystem::remove( strOldFileName );
				std::filesystem::rename( strNewFileName, strOldFileName );
			}
	...
	for (i = 0; i < iCount; i++)
	{
		...
		TRY
		{
			...
			if (!bSkip)
			{
				...
				std::string filename( szPredicted );
				std::transform( filename.begin(), filename.end(), filename.begin(), ::tolower );
				...
				CZipFileHeader* pHeader = zip[nIndex];
				if( pHeader )
				{
					std::filesystem::path path{ filename };
